...
SCENARIO( "the amount of classes can be changed", "[gpa]" ) {
    
    GIVEN ( "the amount of classes is 0" ) {
        
        College mycollege("Richard");
        ifstream in;
        
        WHEN ( "the amount of classes is at least 1 and less than 5" ) {
            in.open("input1.txt");
            mycollege.load(in);
            REQUIRE(!in.fail());
            in.close();
            REQUIRE(mycollege.gpa() == 4.00);
        }
        
        WHEN ( "the amount of classes is at least 5 and less than 10" ) {
            in.open("input2.txt");
            mycollege.load(in);
            REQUIRE(!in.fail());
            in.close();
            REQUIRE(mycollege.gpa() == 3.61);
        }
        
        WHEN ( "the amount of classes is at least 10" ) {
            in.open("input3.txt");
            mycollege.load(in);
            REQUIRE(!in.fail());
            in.close();
            REQUIRE(mycollege.gpa() == 3.47);
        }
    }
}
